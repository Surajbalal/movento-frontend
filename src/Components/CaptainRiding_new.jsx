...
export default function CaptainRiding({ rideData }) {
  ...
  // Route logic for captain
  const routeInfo = useMemo(() => {
    if (!rideData) return { origin: null, destination: null, showRoute: false };

    const status = rideData.status;
    console.log('🚗 Captain Ride Status:', status);

    // Captain: Current → Pickup
    if ((status === 'accepted' || status === 'confirmed') && currentLocation && pickup) {
    ...
    if ((status === 'started' || status === 'ongoing') && pickup && destination) {
  ...
  }, [rideData, currentLocation, pickup, destination]);
    ...
    directionsService.route(
      ...
      (result, status) => {
        if (status === "OK") {
          ...
          console.log('✅ Captain directions loaded:', result);
        } else {
  ...
  const mapCenter = routeInfo.origin || currentLocation || pickup;
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <div style={{
              ...
              borderRadius: "50%",
              backgroundColor: 
                routeInfo.routeType === 'captain_to_pickup' ? '#4285F4' :
                routeInfo.routeType === 'pickup_to_destination' ? '#34A853' :
                '#EA4335'
            }} />
            <span style={{ fontSize: "14px", fontWeight: "500", color: "#333" }}>
              {routeInfo.routeType === 'captain_to_pickup' && '🚗 Heading to pickup'}
              {routeInfo.routeType === 'pickup_to_destination' && '🚙 En route to destination'}
